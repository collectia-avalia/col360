...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "supabase_read_table") {
            const { table, select = "*", filter, order, limit = 10 } = args;
        ...
        if (name === "supabase_update_row") {
            const { table, data, filter_column, filter_value } = args;
            const { data: updated, error } = await supabase.from(table).update(data).eq(filter_column, filter_value).select();
            if (error)
                throw error;
            return {
        ...
        if (name === "supabase_delete_row") {
            ...
            const { error } = await supabase.from(table).delete().eq(filter_column, filter_value);
            if (error)
                throw error;
            return {
